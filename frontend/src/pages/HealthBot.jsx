      ...
      const handleReceive = ({ message, sendId, recieverName,recieverId }) => {
        if (doctor) {
          ...
          setDoctorchat((prevchat) => {
            const isExist = prevchat.find((client) => client.socketId === sendId);
            if (!isExist) {
              return [...prevchat, { socketId: sendId, clientName: recieverName, lastMessage: "i want a favour" }];
            }
            return prevchat;
          });
        }
        ...
        <div className="flex  mx-auto bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div className="w-2/4 border-r border-gray-200 p-4 ">
              ...
              {doctorchat.map((chat, index) => (
                <div onClick={()=>handleClient(chat)}
                  key={index}
                  className={` ${selectedClient===chat.socketId ? "bg-[#64F9FA]":"bg-white"} p-3 rounded-lg mb-2 shadow-md border border-gray-200 cursor-pointer
          ...
          <div className="w-2/3 lg:w-[700px] rounded-xl">
            ...
            <div className="h-96 overflow-y-auto p-6">
              {(chat[user?currentClient:selectedClient] || []).map((message, index) => (
                <div
                  key={index}
                  className={`mb-4 ${
